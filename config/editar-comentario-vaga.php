<?php
	...
	if(isset($_POST['autor']) AND isset($_POST['comentarioEdita']) AND isset($_POST['comentario'])){
		if(empty($_POST['autor']) OR empty($_POST['comentarioEdita']) OR empty($_POST['comentario'])){
			...
			$sql_verifica = "SELECT * FROM tb_comentario_vaga WHERE cd_comentario_vaga = '$comentarioEdita' AND id_autor_comentario_vaga = '$autor'";
			$query_verifica = mysqli_query($mysqli, $sql_verifica);
			$row_verifica = mysqli_fetch_assoc($query_verifica);

			if($comentario == $row_verifica['ds_comentario_vaga']){
				...
				<script>
					...
					$("#alerta-resultado-comentario-vaga").addClass("alert-warning");
				</script>
				<?php
			}else{
				$sql_comentario = "UPDATE tb_comentario_vaga SET ds_comentario_vaga = '$comentario' WHERE cd_comentario_vaga = '$comentarioEdita' AND id_autor_comentario_vaga = '$autor'";
				if($resultado_comentario = $mysqli->query($sql_comentario)){
					$mysqli->query("UPDATE tb_comentario_vaga SET created_comentario_vaga = NOW() WHERE cd_comentario_vaga = '$comentarioEdita' AND id_autor_comentario_vaga = '$autor'");
