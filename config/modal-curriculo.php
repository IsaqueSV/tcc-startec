<?php 
	...
	if(isset($_SESSION['cdUsuario']) AND isset($_POST['curriculoPesquisado'])){
		if(empty($_SESSION['cdUsuario']) OR empty($_POST['curriculoPesquisado'])){
			header('Location: ../index.php');	
		}else{
			$cdUsuario = $_SESSION['cdUsuario'];
			$curriculoPesquisado = $_POST['curriculoPesquisado'];		
		}
	}else{
		header('Location: ../index.php');
	}
	...
	<script>
		...
		$("#btn-denunciar").click(function(){
			$.ajax({
				...
				type: "POST",
				data: "curriculoPesquisado="+"<?php echo $curriculoPesquisado; ?>",
				dataType: "html"
...
<?php
	$sql = "SELECT * FROM tb_curriculo INNER JOIN tb_usuario ON tb_curriculo.id_autor_curriculo = tb_usuario.cd_usuario INNER JOIN tb_pchave_curriculo ON tb_curriculo.id_pchave_curriculo = tb_pchave_curriculo.cd_pchave_curriculo WHERE cd_curriculo = '$curriculoPesquisado'";
		      	...
		      	<div style='width: 100%; border: 2px solid #DCDCDC; border-radius: 8px; padding: 3%;'>
			    	<table>
			    		<tbody>
			    			<tr>
				    			<td>
				    			Título: ".$row['nm_curriculo']."
				    			</td>
				    			<td style='text-align: right'>
				    				".date("d/m/Y H:i", strtotime($row['created_curriculo']))."
				    			</td>
			    			</tr>
			    			<tr>
			    				<td>
				    			Especialização: ".$row['nm_pchave_curriculo']."
				    			</td>
			    			</tr>
			    			<tr>	
