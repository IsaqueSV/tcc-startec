<?php
	...
	if(isset($_GET['see'])){
		...
		$sql_verifica = "SELECT * FROM tb_vaga WHERE ds_url_vaga = '$url'";
		...
		if($num_verifica > 0){
			...
			<head>
				...
				-->
				<title>Denuncias - <?php echo $row_dados['nm_vaga']; ?></title>
					...
					$(document).ready(function(){
						...
						$(".btn-visualizar-motivo").click(function(){
							...
							$.ajax({
								...
								type: "POST",
								data: "idDenuncia="+$id_denuncia + "&nivelMotivo="+"4",
								dataType: "html"
							}).done(function(resposta){
			...
			<body>
				...
				<!-- Título (barra) -->
				<div id="titulo">
					<label class="h5" id="titulo-label">DENUNCIAS VAGA</label>
					...
					<div style="width: 100%; display: flex; margin-bottom: 0.9%;">
						<div style="width: 100%; text-align: center; display: flex;">
							<div title="ID denunciado" style="width: 6%; padding-top: 1.3%; padding-bottom: 1.3%; border: 1px solid;">
								<?php echo $row_dados['cd_vaga']; ?>
							</div>
							<div title="Nome do currículo" style="width: 82%; padding-top: 1.3%; padding-bottom: 1.3%; border: 1px solid; border-left: 0;">
								<?php echo $row_dados['nm_vaga']; ?>
							</div>
							<div class="btn-deletar-vaga btn btn-light" id="<?php echo $row_dados['ds_url_vaga']; ?>" title="Deletar" style="border-radius: 0; width: 12%; padding-top: 1.3%; padding-bottom: 1.3%; border: 1px solid; border-left: 0;">
								Deletar
							</div>
						</div>
					</div>
					<?php
						$sql_denuncias = "SELECT * FROM tb_denuncia_vaga WHERE id_denuncia_vaga = '".$row_dados['cd_vaga']."'";
						...
						if($num_denuncias > 0){
							...
							while($row_denuncias = mysqli_fetch_assoc($query_denuncias)){
								$sql_autor_da_denuncia = "SELECT * FROM tb_usuario WHERE cd_usuario = '".$row_denuncias['id_autor_denuncia_vaga']."'";
								...
								?>
									<div style="width: 100%; display: flex;">
										...
										<div style="width: 32%; text-align: center; display: flex;">
											<div class="btn-visualizar-motivo btn btn-light" id="<?php echo $row_denuncias['cd_denuncia_vaga']; ?>" title="Visualizar" style="border-radius: 0; width: 62.5%; padding-top: 4%; border-right: 1px solid; border-bottom: 1px solid;">
												Visualizar
											</div>
											<div class="btn-deletar-denuncia btn btn-light" id="<?php echo $row_denuncias['cd_denuncia_vaga']; ?>" title="Deletar" style="border-radius: 0; width: 37.5%; padding-top: 4%; border-bottom: 1px solid;">
												Deletar
											</div>
										</div>
									</div>
							...
							?>
								<div id="div-projeto" style="width: 100%; margin: 0; border-radius: 0; background-color: white; color: black; border: 1px solid;">
									<label class='h6'>Essa vaga não possui nenhuma denúncia!</label>
								</div>
							<?php
						}
					?>
