<?php
	...
	$sql_empresas = "SELECT * FROM tb_usuario WHERE id_nivel = 2";
	$query_empresas = mysqli_query($mysqli, $sql_empresas);
	$num_empresas = mysqli_num_rows($query_empresas);
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<!-- Título (barra) -->
	<div id="titulo">
		<label class="h5" id="titulo-label">EMPRESAS</label>
	...
	<div class="input-group mb-3" id="div-pesquisa">
		<select class="form-select form-select-sm p-3" id="select-pesquisa">
			<option value="2" selected title="Filtrar busca">Empresa</option>
		</select>
	...
	<div id="lista-de-usuarios" style="width: 80%; margin-top: 5%; margin-left: 10%; margin-right: 10%;border-radius: 8px;">
		<?php
			if($num_empresas > 0){
				while($row_empresa = mysqli_fetch_assoc($query_empresas)){
					?>
						<div style="width: 100%; display: flex; margin-bottom: 0.5%;">
							<div style="width: 60%; text-align: center; display: flex;">
								<div title="ID empresa" style="width: 10%; padding-top: 2%; background-color: white; padding-bottom: 2%; border: 1px solid; border-top-left-radius: 4px; border-bottom-left-radius: 4px;">
									<?php echo $row_empresa['cd_usuario']; ?>
								</div>
								<div title="Nome da empresa" style="width: 90%; padding-top: 2%; padding-bottom: 2%; border: 1px solid; background-color: white; border-left: 0;">
									<?php echo $row_empresa['nm_usuario']; ?>
								</div>
							</div>
							<div style="width: 40%; text-align: center; display: flex;">
								<div class="btn-denuncias-usuario btn btn-light" id="<?php echo $row_empresa['ds_url_usuario']; ?>" title="Denuncias" style="border-radius: 0; width: 30%; padding-top: 2.5%; padding-bottom: 2%; border: 1px solid; border-left: 0;">
									Denuncias
								</div>
								<div class="btn-visualizar-usuario btn btn-light" id="<?php echo $row_empresa['ds_url_usuario']; ?>" title="Visualizar" style="border-radius: 0; width: 40%; padding-top: 2.5%; padding-bottom: 2%; border: 1px solid; border-left: 0;">
									Visualizar
								</div>
								<div class="btn-deletar-usuario btn btn-light" id="<?php echo $row_empresa['ds_url_usuario']; ?>" title="Deletar" style="border-radius: 0; width: 30%; padding-top: 2.5%; padding-bottom: 2%; border: 1px solid; border-left: 0; border-top-right-radius: 4px; border-bottom-right-radius: 4px;">
									Deletar
								</div>
							</div>
						</div>
